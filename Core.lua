...
function ShellcoinTicker:InitializeDB()
    ...
    -- Run DB historical compaction on initialization
    self:CompactHistory()
end
...
function ShellcoinTicker:UpdateHistoryAndChange(price)
    ...
    if shouldInsert then
        ...
        -- Compact history (which automatically handles 1-year pruning and progressive compression)
        -- Throttled to run at most once every 10 minutes (600s) during active gameplay
        if not ShellcoinTicker.lastCompactionTime or (time() - ShellcoinTicker.lastCompactionTime > 600) then
            ShellcoinTicker:CompactHistory()
            ShellcoinTicker.lastCompactionTime = time()
        end
    end
end

-- Compacts the historical data progressively to keep the SavedVariables file size small
-- while preserving extreme prices (high/low peaks) within each time bucket.
function ShellcoinTicker:CompactHistory()
    if not ShellcoinTickerDB or not ShellcoinTickerDB.history then return end
    
    local now = time()
    local history = ShellcoinTickerDB.history
    local numEntries = table.getn(history)
    if numEntries <= 1 then return end
    
    local cutoff10Y = now - 315360000 -- 10 years ago (delete older)
    local cutoff30D = now - 2592000 -- 30 days ago
    local cutoff7D = now - 604800   -- 7 days ago
    local cutoff24H = now - 86400   -- 24 hours ago
    
    local buckets12H = {} -- for 30D to 10Y
    local buckets2H = {}  -- for 7D to 30D
    local buckets30M = {} -- for 24H to 7D
    local rawPoints = {}  -- for last 24H
    ...
    for i = 1, numEntries do
        ...
        if entry and type(entry) == "table" and entry.time and entry.price then
            local t = entry.time
            if t >= cutoff24H then
                table.insert(rawPoints, entry)
            elseif t >= cutoff7D then
                local bIdx = math.floor((t - cutoff7D) / W_30M) + 1
                if not buckets30M[bIdx] then buckets30M[bIdx] = {} end
                table.insert(buckets30M[bIdx], entry)
            elseif t >= cutoff30D then
                local bIdx = math.floor((t - cutoff30D) / W_2H) + 1
                if not buckets2H[bIdx] then buckets2H[bIdx] = {} end
                table.insert(buckets2H[bIdx], entry)
            elseif t >= cutoff10Y then
                local bIdx = math.floor((t - cutoff10Y) / W_12H) + 1
                if not buckets12H[bIdx] then buckets12H[bIdx] = {} end
                table.insert(buckets12H[bIdx], entry)
            end
        end
    end
    ...
    local function processBuckets(bucketArray)
        local keys = {}
        for k in pairs(bucketArray) do
            table.insert(keys, k)
        end
        table.sort(keys)
        
        for idx = 1, table.getn(keys) do
            local pts = bucketArray[keys[idx]]
            local count = table.getn(pts)
            if count > 0 then
                local minPt = pts[1]
                local maxPt = pts[1]
                for j = 2, count do
                    if pts[j].price < minPt.price then minPt = pts[j] end
                    if pts[j].price > maxPt.price then maxPt = pts[j] end
                end
                
                local temp = {}
                table.insert(temp, minPt)
                if maxPt.time ~= minPt.time then
                    table.insert(temp, maxPt)
                end
                
                table.sort(temp, function(a, b) return a.time < b.time end)
                
                for j = 1, table.getn(temp) do
                    table.insert(compacted, temp[j])
                end
            end
        end
    end
    
    processBuckets(buckets12H)
    processBuckets(buckets2H)
    processBuckets(buckets30M)
    
    for i = 1, table.getn(rawPoints) do
        table.insert(compacted, rawPoints[i])
    end
    
    table.sort(compacted, function(a, b) return a.time < b.time end)
    
    ShellcoinTickerDB.history = compacted
end
